...
class CharFrame:
	...
	# 每个字符的覆盖范围是256／len()，灰度值／每个字符覆盖范围＝字符索引
    ...
    def convert(self, img, limitSize=-1, fill=False, wrap=False):
        ...
        if wrap:
            blank += '\n'
        for i in range(img.shape[0]):	#行数
            for j in range(img.shape[1]):#列数
...
class V2Char(CharFrame):
    ...
    def __init__(self, path):
        if path.endswith('txt'):	#表明输入是导出文件
            self.load(path)
        else:				#表明输入是视频文件
    ...
    def genCharVideo(self, filepath):	
        ...
        self.timeInterval = round(1/cap.get(5), 3)	#表明保留3位小数
        nf = int(cap.get(7))
    ...
    def play(self, stream = 1):
        ...
        def getChar():
            nonlocal breakflag	# 函数作用域中使用外层变量（非全局）	
